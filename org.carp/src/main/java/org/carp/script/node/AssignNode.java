...
public class AssignNode extends BaseNode{
	...
	public void initValue(Node node) throws Exception{
		this.var = this.getAttrValue(node, "var", null);
		String value = this.getAttrValue(node, "value", null);
		String test = this.getAttrValue(node, "test", null);
		if(var ==null || value == null)
			throw new CarpException("assign tag: var cannot be null, value cannot be null, But: [var="+var+"]; [value="+value+"]");
	...
	public BaseNode onClone() {
		AssignNode textnode = new AssignNode();
		textnode.var = this.var;
